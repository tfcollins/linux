				    ...
				    uint8_t pin_enable, ad9208_pdn_mode_t pin_mode)
{
	...
	if (!pin_enable) {
		err = ad9208_register_write(h,
					    AD9208_CHIP_PIN_CTRL0_REG, (uint8_t) AD9208_CHIP_PDN_PIN_DISABLE);
		if (err != API_ERROR_OK)
			return err;
	} else {
		err = ad9208_register_write(h,
		...
		if (err != API_ERROR_OK)
			return err;
		err = ad9208_register_read(h, AD9208_CHIP_PIN_CTRL1_REG, &tmp_reg);
		if (err != API_ERROR_OK)
			return err;
		tmp_reg &= (~AD9208_CHIP_PDN_MODE(ALL));
		tmp_reg |= ((pin_mode == AD9208_STANDBY) ? 0x1 : 0x0);
		err = ad9208_register_write(h, AD9208_CHIP_PIN_CTRL1_REG, tmp_reg);
		if (err != API_ERROR_OK)
			return err;
	}
...
#endif
	err = ad9208_register_write(h, AD9208_DEV_CFG_REG, AD9208_PDN_MODE(mode));
	if (err != API_ERROR_OK)
		return err;
				     ...
				     uint8_t div)
{
	...
	if ((fs_hz > AD9208_ADC_CLK_MAX_HZ) ||
	    (fs_hz < AD9208_ADC_CLK_MIN_HZ))
		return API_ERROR_INVALID_PARAM;
