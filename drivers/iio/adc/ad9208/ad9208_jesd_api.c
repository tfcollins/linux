...
int ad9208_jesd_set_if_config(ad9208_handle_t *h,
				      jesd_param_t jesd_param, uint64_t *lane_rate_kbps)
{
	int err;
	...
	if ((slr_mbps > LANE_RATE_MAX_MBPS) ||
	    (slr_mbps < LANE_RATE_MIN_MBPS)) {
	...
	err = ad9208_register_write(h,
				    AD9208_JESD_M_CFG_REG, (jesd_param.jesd_M - 1));
	if (err != API_ERROR_OK)
		return err;
	...
	err = ad9208_register_write(h,
				    AD9208_JESD_CS_N_CFG_REG, tmp_reg);
	if (err != API_ERROR_OK)
		return err;
	...
	err = ad9208_register_write(h,
				    AD9208_JESD_SCV_NP_CFG_REG, tmp_reg);
	if (err != API_ERROR_OK)
		return err;

	/*Octets per Frame (F) and Frames per Multiframe (k)*/
	err = ad9208_register_write(h,
				    AD9208_JESD_F_CFG_REG, (AD9208_JESD_F(jesd_param.jesd_F - 1)));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_JESD_K_CFG_REG, (AD9208_JESD_K(jesd_param.jesd_K - 1)));
	if (err != API_ERROR_OK)
		return err;
	/*Set Lanes*/
				      ...
				      jesd_param_t *jesd_param)
{
	...
	for (i = 0; i < AD9208_JESD_CFG_REG_OFFSET; i++) {
		err = ad9208_register_read(h, (AD9208_JESD_L_SCR_CFG_REG + i), &tmp_reg[i]);
		if (err != API_ERROR_OK)
			return err;
	}
	...
	for (i = 0; i < AD9208_JESD_ID_CFG_REG_OFFSET; i++) {
		err = ad9208_register_read(h, (AD9208_JESD_DID_CFG_REG + i), &tmp_reg[i]);
		if (err != API_ERROR_OK)
			return err;
	}
...
int ad9208_jesd_get_lane_xbar(ad9208_handle_t *h, uint8_t *phy_log_map)
{
	...
	for (i = 0; i < AD9208_JESD_XBAR_CFG_REG_OFFSET; i++) {
		err = ad9208_register_read(h, AD9208_JESD_XBAR_CFG_REG + i, &tmp_reg[i]);
		if (err != API_ERROR_OK)
			return err;
	}
...
int ad9208_jesd_syref_config_set(ad9208_handle_t *h,
		uint8_t sysref_edge_sel, uint8_t clk_edge_sel, uint8_t neg_window_skew,
...
int ad9208_jesd_syref_status_get(ad9208_handle_t *h,
		uint8_t *hold_status, uint8_t *setup_status, uint8_t *phase_status)
{
	int err;
	uint8_t tmp_reg;
...
int ad9208_jesd_sysref_timestamp_set(ad9208_handle_t *h,
		uint8_t timestamp_en, uint8_t control_bit, uint8_t delay)
{
	int err;
	...
	if ((timestamp_en > 0x1) ||
	    (control_bit > 2) ||
	    (delay > AD9208_SYSREF_TS_DELAY(ALL)))
		return API_ERROR_INVALID_PARAM;
	...
	if (timestamp_en == 0x1) {
		...
		tmp_reg |= ((AD9208_OP_CONV_CTRL_BIT_SEL(AD9208_CB_SYSREF)) <<
			    (tmp_nibble * 4));
