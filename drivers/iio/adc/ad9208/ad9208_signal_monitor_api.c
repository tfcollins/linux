		...
		uint16_t upper_dbfs, uint16_t lower_dbfs, uint16_t dwell_time)
{
	...
	err = ad9208_register_write(h,
				    AD9208_FD_DWELL_LSB_REG, AD9208_FD_DWELL_LSB(dwell_time));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_FD_DWELL_MSB_REG, AD9208_FD_DWELL_MSB(dwell_time));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_FD_UT_LSB_REG, AD9208_FD_UT_LSB(upper_dbfs));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_FD_UT_MSB_REG, AD9208_FD_UT_MSB(upper_dbfs));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_FD_UT_LSB_REG, AD9208_FD_LT_LSB(lower_dbfs));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_FD_LT_MSB_REG, AD9208_FD_LT_MSB(lower_dbfs));
	if (err != API_ERROR_OK)
		return err;
