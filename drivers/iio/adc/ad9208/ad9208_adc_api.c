...
static void adi_api_utils_mult_128(uint64_t a, uint64_t b, uint64_t *hi, uint64_t *lo)
{
...
static int ad9208_get_dec_filter_cfg(ad9208_adc_data_frmt_t op_data_format,
				     uint8_t dcm_rate,
				     uint8_t *filt_sel_val_0, uint8_t *filt_sel_val_1)
{
	uint8_t i = 0;
				      ...
				      ad9208_adc_scale_range_t full_scale_range)
{
	...
	err = ad9208_register_write(h,
				    AD9208_FULL_SCALE_CFG_REG, AD9208_TRM_VREF(fs_val));
	if (err != API_ERROR_OK)
		return err;
...
int ad9208_adc_set_input_cfg(ad9208_handle_t *h,
				     ...
				     uint8_t ext_vref, ad9208_adc_scale_range_t full_scale_range)
{
	int err;
	uint8_t tmp_reg;
	if (h == NULL)
...
int ad9208_adc_set_input_buffer_cfg(ad9208_handle_t *h,
		...
		ad9208_adc_buff_curr_t buff_curr_p, ad9208_adc_buff_curr_t vcm_buff)
{
	int err;
	uint8_t tmp_reg;
	if (h == NULL)
	...
	err = ad9208_register_write(h,
				    AD9208_BUFF_CFG_P_REG, AD9208_BUFF_CTRL_P(buff_curr_p));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h,
				    AD9208_BUFF_CFG_N_REG, AD9208_BUFF_CTRL_N(buff_curr_n));
	if (err != API_ERROR_OK)
		return err;
...
int ad9208_adc_set_data_format(ad9208_handle_t *h,
				       ad9208_adc_data_frmt_t ip_data_frmt, ad9208_adc_data_frmt_t op_data_frmt)
{
	int err,  offset;
	uint8_t tmp_reg, i;
	if (h == NULL)
	...
	for (i = 0; i < AD9208_NOF_FC_MAX; i++) {
		...
		tmp_reg |= (ip_data_frmt == AD9208_DATA_FRMT_COMPLEX) ? AD9208_DDCX_MIXER_SEL :
			   0;
...
int ad9208_adc_set_ddc_ip_mux(ad9208_handle_t *h,
				      uint8_t ddc_ch, ad9208_adc_ch_t i_data_src, ad9208_adc_ch_t q_data_src)
{
	int err,  offset;
	uint8_t tmp_reg;
	if (h == NULL)
	...
	err = ad9208_register_write(h, AD9208_DDCX_DATA_SEL_REG + offset, tmp_reg);
	if (err != API_ERROR_OK)
		return err;
		...
		uint64_t po)
{
	...
	if (ddc_ch >= AD9208_NOF_FC_MAX)
		return API_ERROR_INVALID_PARAM;
	offset = (AD9208_DDCX_REG_OFFSET * ddc_ch);
	err = ad9208_register_write(h, (AD9208_DDCX_PO0_REG + offset) , ADI_GET_BYTE(po,
				    0));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h, (AD9208_DDCX_PO1_REG + offset), ADI_GET_BYTE(po,
				    8));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h, (AD9208_DDCX_PO2_REG + offset), ADI_GET_BYTE(po,
				    16));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h, (AD9208_DDCX_PO3_REG + offset), ADI_GET_BYTE(po,
				    24));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h, (AD9208_DDCX_PO4_REG + offset), ADI_GET_BYTE(po,
				    32));
	if (err != API_ERROR_OK)
		return err;
	err = ad9208_register_write(h, (AD9208_DDCX_PO5_REG + offset), ADI_GET_BYTE(po,
				    40));
	if (err != API_ERROR_OK)
		return err;
